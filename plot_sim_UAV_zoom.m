function plot_sim_UAV_zoom(fig,sim )
...
%             plot3_poly( [sim.Cd{i} ; zeros(size(sim.Cd{i}(1,:)))], 'c--');
        ...
        for i=1:sim.N
            ...
            quadrotor_plot(fig, sim.x_quad(1:3,i), sim.x_quad(7:9,i) );
            plot3( [sim.x_quad(1,i) sim.x_quad(1,i)], [sim.x_quad(2,i) sim.x_quad(2,i)], ...
                [sim.x_quad(3,i) 0], 'k--.' )
            if length(sim.W{i})>0
                for j=1:24:length(sim.W{i}(1,:))
                    plot3([sim.W{i}(1,j) sim.x_quad(1,i)], [sim.W{i}(2,j) sim.x_quad(2,i)],...
                        [0 sim.x_quad(3,i)], 'r--');
                end
            end
        end
