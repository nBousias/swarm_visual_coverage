function uZ = control_uniform_altitude(region, W, Cgs,C_maxz, f, df, i, Jz_maxz,Jz_minz)
...
if ~isempty(W{i})
    ...
    for k=1:length(Wi(1,:))-1
        ...
        if ~(onB1 && onB2)
            % Check if any of them is on Cgs_i, if not, dont integrate
            [~, onCi1] = inpolygon( pt1(1), pt1(2), Cgs{i}(1,:), Cgs{i}(2,:) );
            [~, onCi2] = inpolygon( pt2(1), pt2(2), Cgs{i}(1,:), Cgs{i}(2,:) );

            if (onCi1 || onCi2)
                % Check if they are both inside Cgs_j for all j in overlap
...
%--------------------------------------------------------------------------
                            ...
                            if q==2
                                uZ = uZ + (f(i)-f(j)) * dot(Jz_maxz(pt1), nvector);
                            else
                                uZ = uZ + (f(i)-f(j)) * dot(Jz_minz(pt1), nvector);
                            end
                        end
...
%--------------------------------------------------------------------------
                    ...
                    if q==2
                        uZ = uZ + f(i) * dot(Jz_maxz(pt1), nvector);
                    else
                        uZ = uZ + f(i) * dot(Jz_minz(pt1), nvector);
                    end
                end
